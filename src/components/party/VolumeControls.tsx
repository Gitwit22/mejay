import { Volume2, Shield, Gauge, Sparkles } from 'lucide-react';
...
export function VolumeControls() {
  ...
  return (
    <>
      <GatedSection feature="autoVolume" label="Upgrade for Auto Volume">
      ...
      <div className="glass-card mt-4">
        ...
        <div className="flex gap-2">
          {([
            ...
            <button
              key={preset.key}
              onClick={() => updateUserSettings({ vibesPreset: preset.key })}
              className={cn(
                'flex-1 py-2 rounded-lg text-xs font-medium transition-colors',
                settings.vibesPreset === preset.key
        ...
        <p className="text-[10px] text-muted-foreground mt-2">
          Smooth EQ templates with pre-limiter trim.
        </p>
      </div>
    </>
  );
