...
export const onRequest = async (ctx: {request: Request; env: Env}): Promise<Response> => {
  ...
  try {
    ...
    if (!user) {
      if (purpose === 'password_reset') return json({ok: false, error: 'user_not_found'}, {status: 400})
      const userId = crypto.randomUUID()
      await env.DB
        .prepare('INSERT INTO users (id, email, password_hash, updated_at) VALUES (?1, ?2, ?3, ?4)')
